...
public class VariableBlurUIView: UIVisualEffectView {
    public init(
        ...
        let filterClassString: String = {
            ...
            print("[TabBarVariableBlurView] couldn't decode the filter class string.")
            return ""
        }()
        ...
        let filterWithTypeString: String = {
            ...
            print("[TabBarVariableBlurView] couldn't decode the filter method string.")
            return ""
        }()
        ...
        guard let filterClass = NSClassFromString(filterClassString) as AnyObject as? NSObjectProtocol else {
            print("[TabBarVariableBlurView] couldn't create CAFilter class.")
            return
        }

        /// Make sure the filter class responds to the selector.
        guard filterClass.responds(to: filterWithTypeSelector) else {
            print("[TabBarVariableBlurView] Doesn't respond to selector \(filterWithTypeSelector)")
            return
        }
        ...
        guard let variableBlur = variableBlur as? NSObject else {
            print("[TabBarVariableBlurView] Couldn't cast the blur filter.")
            return
        }
    ...
    required init?(coder: NSCoder) {
        ...
        // We can't properly initialize with a coder since we need specific parameters
        // This implementation allows the class to be used in Interface Builder if needed
        return nil
    }
...
/// A variable blur view.
public struct TabBarVariableBlurView: UIViewRepresentable {
