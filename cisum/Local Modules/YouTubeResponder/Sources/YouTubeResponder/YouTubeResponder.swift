...
public class YouTube {
    ...
    public func checkAvailability() async throws {
        ...
        for reason in messages {
            switch status {
            case .unplayable:
                if reason?.starts(with: "Join this channel to get access to members-only content") ?? false {
                    throw YouTubeResponderError.membersOnly
                }
            case .loginRequired:
                if reason.map({ $0.starts(with: "This is a private video") || $0.starts(with: "This video is private") }) ?? false {
                    throw YouTubeResponderError.videoPrivate
                }
            case .error:
                throw YouTubeResponderError.videoUnavailable
            case .liveStream:
                let streamingData = try await videoInfos.map { $0.streamingData }
                if streamingData.allSatisfy({ $0?.hlsManifestUrl == nil }) {
                    throw YouTubeResponderError.liveStreamError
                }
                continue
            case .ok, .none:
                continue
        ...
        get async throws {
            ...
            if !streamingData.isEmpty {
                ...
                if !streamingData.isEmpty {
                    ...
                    throw YouTubeResponderError.extractError
                }
            }
        }
    }
    ...
    var videoInfos: [InnerTube.VideoInfo] {
        get async throws {
            ...
            if videoInfos.isEmpty {
                throw errors.first ?? YouTubeResponderError.extractError
            }
    ...
    private func bypassAgeGate() async throws {
        ...
        if innertubeResponse.playabilityStatus?.status == "UNPLAYABLE" || innertubeResponse.playabilityStatus?.status == "LOGIN_REQUIRED" {
            throw YouTubeResponderError.videoAgeRestricted
        }
