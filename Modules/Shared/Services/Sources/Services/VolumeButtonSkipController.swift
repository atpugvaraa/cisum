...
@Observable
@MainActor
public final class VolumeButtonSkipController {
    public static let shared = VolumeButtonSkipController()
    ...
    private weak var playerViewModel: AnyObject? // Use AnyObject to avoid circular dependency if needed, or import Player module if it's fine.
    // Actually, I can't import Player here because Player depends on Services.
    // I should use a protocol or closure.
    
    public var onSkipForward: (() -> Void)?
    public var onSkipBackward: (() -> Void)?
    public var isPlaying: () -> Bool = { false }
    public var canSkipForward: () -> Bool = { false }
    public var canSkipBackward: () -> Bool = { false }
    ...
    public func configure(volumeController: SystemVolumeController = .shared) {
    ...
    private func canSkip(for direction: Direction) -> Bool {
        ...
        return skipForward ? canSkipForward() : canSkipBackward()
    }
